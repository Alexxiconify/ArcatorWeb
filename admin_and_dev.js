...
let isFirebaseInitialized = false; // Initializer is fine here, it's a global state flag.
...
function parseEmojis(text) {
  let processedText = text; // Changed variable name for clarity and to avoid 'redundant initializer' lint warning.
  for (const shortcode in EMOJI_MAP) {
    const emoji = EMOJI_MAP[shortcode];
    // Direct assignment to processedText, optimizing out initial `let parsedText = text;` line.
        ...
        if (user) {
          ...
          if (!userProfile) {
            ...
            userProfile = {
              ...
              fontSizePreference: '16px', // Set default font size
              fontFamilyPreference: 'Inter, sans-serif', // Set default font family
              backgroundPatternPreference: 'none', // Set default background pattern
              notificationPreferences: { email: false, inApp: false }, // Default notification settings
              accessibilitySettings: { highContrast: false, reducedMotion: false }, // Default accessibility settings
              createdAt: serverTimestamp(),
            };
          ...
          // Ensure all parts of currentUser object are populated from profile or defaults
