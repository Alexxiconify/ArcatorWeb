...
import {
  ...
  getUserProfileFromFirestore, DEFAULT_PROFILE_PIC
...
async function createTempPage(title, content) {
  ...
  const allowComments = document.getElementById('temp-page-allow-comments-create')?.checked || false;
  ...
  try {
    ...
    if (document.getElementById('temp-page-allow-comments-create')) {
      document.getElementById('temp-page-allow-comments-create').checked = false;
    }
...
async function renderTempPages(tempPages) {
  ...
  tbody.innerHTML = tempPages.map(page => `
    <tr class="hover:bg-table-row-even-bg transition-colors">
      ...
      <td class="px-2 py-1 text-text-secondary text-xs">
        <div class="flex space-x-1">
          <button onclick="openEditTempPageModal('${page.id}', '${escapeHtml(page.title)}', '${escapeHtml(page.content)}', ${page.allowComments !== false})" 
...
function openEditTempPageModal(id, title, content, allowComments) {
  currentEditingTempPageId = id;
  editTempPageTitleInput.value = title;
  editTempPageContentInput.value = content;
  
  // Set default values for new fields
  document.getElementById('edit-temp-page-status').value = 'active';
  document.getElementById('edit-temp-page-visibility').value = 'public';
  document.getElementById('edit-temp-page-allow-comments').checked = allowComments;
  document.getElementById('edit-temp-page-show-author').checked = true;
  document.getElementById('edit-temp-page-enable-tracking').checked = false;
  document.getElementById('edit-temp-page-meta-description').value = '';
  document.getElementById('edit-temp-page-custom-css').value = '';
  document.getElementById('edit-temp-page-custom-js').value = '';
...
async function loadDMHistory() {
  ...
  if (!tbody) return;
  try {
    // Load all user profiles first to get their DM collections
    const usersRef = collection(db, `artifacts/arcator-web/public/data/user_profiles`);
    const usersSnapshot = await getDocs(usersRef);
    const allDmData = [];
    
    // Iterate through each user to get their DMs
    for (const userDoc of usersSnapshot.docs) {
      const userId = userDoc.id;
      const userData = userDoc.data();
      
      // Get DMs for this user
      const userDmsRef = collection(db, `artifacts/arcator-web/users/${userId}/dms`);
      try {
        const dmsSnapshot = await getDocs(userDmsRef);
        dmsSnapshot.forEach(dmDoc => {
        ...
        console.warn(`Could not load DMs for user ${userId}:`, error);
      }
    }
    
    if (allDmData.length === 0) {
    ...
    tbody.innerHTML = allDmData.map(dm => `
      <tr>
        <td class="px-2 py-1 text-text-primary text-xs">${dm.createdAt ? new Date(dm.createdAt.toDate()).toLocaleString() : ''}</td>
        <td class="px-2 py-1 text-text-primary text-xs">${escapeHtml(dm.from || dm.userName || '')}</td>
...
async function loadFormsData() {
  ...
  if (!tbody) return;
  try {
    // Load thematas from the correct path
    const thematasRef = collection(db, `artifacts/arcator-web/public/data/thematas`);
    const thematasSnapshot = await getDocs(thematasRef);
    const formsData = [];
    
    // Iterate through each thema to get threads and comments
    for (const themaDoc of thematasSnapshot.docs) {
      const themaId = themaDoc.id;
      const themaData = themaDoc.data();
      
      // Get threads for this thema
      const threadsRef = collection(db, `artifacts/arcator-web/public/data/thematas/${themaId}/threads`);
      try {
        ...
        for (const threadDoc of threadsSnapshot.docs) {
          ...
          const commentsRef = collection(db, `artifacts/arcator-web/public/data/thematas/${themaId}/threads/${threadId}/comments`);
          try {
            const commentsSnapshot = await getDocs(commentsRef);
            const commentCount = commentsSnapshot.size;
            
            formsData.push({
              id: `${themaId}_${threadId}`,
              formType: themaData.name || 'Unknown Thema',
              theme: themaData.theme || 'default',
              comment: commentCount > 0,
              submissions: commentCount + 1, // +1 for the initial thread
              createdAt: threadData.createdAt,
              title: threadData.title || 'Untitled Thread',
              author: threadData.author || 'Unknown'
            });
          } catch (error) {
            console.warn(`Could not load comments for thread ${threadId}:`, error);
            // Still add the thread even if comments fail to load
            formsData.push({
              id: `${themaId}_${threadId}`,
              formType: themaData.name || 'Unknown Thema',
              theme: themaData.theme || 'default',
        ...
        console.warn(`Could not load threads for thema ${themaId}:`, error);
      }
    }
    ...
    tbody.innerHTML = formsData.map(form => `
      <tr>
        ...
        <td class="px-2 py-1 text-text-primary text-xs">${form.submissions}</td>
        <td class="px-2 py-1 text-text-primary text-xs"><button class="btn-primary btn-blue text-xs">View</button></td>
      </tr>
    `).join('');
  } catch (e) {
...
function setupEventListeners() {
  ...
  saveTempPageChangesBtn?.addEventListener('click', async () => {
    ...
    const status = document.getElementById('edit-temp-page-status').value;
    const visibility = document.getElementById('edit-temp-page-visibility').value;
    const allowComments = document.getElementById('edit-temp-page-allow-comments').checked;
    const showAuthor = document.getElementById('edit-temp-page-show-author').checked;
    const enableTracking = document.getElementById('edit-temp-page-enable-tracking').checked;
    const metaDescription = document.getElementById('edit-temp-page-meta-description').value.trim();
    const customCSS = document.getElementById('edit-temp-page-custom-css').value.trim();
    const customJS = document.getElementById('edit-temp-page-custom-js').value.trim();
  ...
  // Cancel temp page changes
  const cancelTempPageChangesBtn = document.getElementById('cancel-temp-page-changes-btn');
  cancelTempPageChangesBtn?.addEventListener('click', () => {
    editTempPageModal.style.display = 'none';
    currentEditingTempPageId = null;
  });

  // Temp page edit form
  const editTempPageForm = document.getElementById('edit-temp-page-form');
  editTempPageForm?.addEventListener('submit', async function(event) {
    event.preventDefault();
    if (!currentEditingTempPageId) {
      console.error("ERROR: currentEditingTempPageId is null. Cannot save changes.");
      return;
...
function openEditTempPageModal(id, title, content) {
  ...
  document.getElementById('edit-temp-page-status').value = 'active';
  document.getElementById('edit-temp-page-visibility').value = 'public';
  document.getElementById('edit-temp-page-allow-comments').checked = true;
  document.getElementById('edit-temp-page-show-author').checked = true;
  document.getElementById('edit-temp-page-enable-tracking').checked = false;
  document.getElementById('edit-temp-page-meta-description').value = '';
  document.getElementById('edit-temp-page-custom-css').value = '';
