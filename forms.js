...
function showDmTab() {
  document.getElementById('thema-all-tab-content').style.display = 'none';
  document.getElementById('dm-tab-content').style.display = 'block';
  renderDMList();
}
function showThemaTab() {
  document.getElementById('thema-all-tab-content').style.display = 'block';
  document.getElementById('dm-tab-content').style.display = 'none';
}
...
function renderDMList() {
  ...
  if (!window.auth.currentUser || !window.db || !dmList) return;
  const userId = window.auth.currentUser.uid;
  const dmsCol = collection(window.db, `artifacts/${window.appId}/users/${userId}/dms`);
  if (unsubscribeDmList) unsubscribeDmList();
  unsubscribeDmList = onSnapshot(dmsCol, (snapshot) => {
    dmList.innerHTML = '';
    if (snapshot.empty) {
      dmList.innerHTML = '<div class="card p-4 text-center">No DMs yet. Start a new DM!</div>';
      return;
    }
    snapshot.forEach(docSnap => {
      const dm = docSnap.data();
      const div = document.createElement('div');
      div.className = 'dm-item card p-3 mb-2 cursor-pointer';
      div.innerHTML = `<b>${dm.groupName || dm.participants?.filter(p => p !== userId)[0] || 'DM'}</b><br><span class='text-xs text-gray-400'>${dm.lastActivity ? new Date(dm.lastActivity.toDate()).toLocaleString() : ''}</span>`;
      div.onclick = () => openDM(docSnap.id, dm);
      dmList.appendChild(div);
    });
  });
}

// --- Open DM and Render Messages ---
function openDM(dmId, dmData) {
  currentDmId = dmId;
  document.getElementById('conversation-messages-section').style.display = 'block';
  renderDMMessages(dmId);
}

function renderDMMessages(dmId) {
  const messagesUl = document.getElementById('conversation-messages');
  if (!window.auth.currentUser || !window.db || !messagesUl) return;
  const userId = window.auth.currentUser.uid;
  const messagesCol = collection(window.db, `artifacts/${window.appId}/users/${userId}/dms/${dmId}/messages`);
  if (unsubscribeDmMessages) unsubscribeDmMessages();
  unsubscribeDmMessages = onSnapshot(messagesCol, (snapshot) => {
    messagesUl.innerHTML = '';
    snapshot.forEach(docSnap => {
      const msg = docSnap.data();
      const li = document.createElement('li');
      li.className = 'message-item' + (msg.createdBy === userId ? ' own-message' : ' other-message');
      li.innerHTML = `<div>${msg.content}</div><div class='text-xs text-gray-400'>${msg.creatorDisplayName || ''} ${msg.createdAt ? new Date(msg.createdAt.toDate()).toLocaleString() : ''}</div>`;
...
// --- Send DM Message ---
document.getElementById('conversation-send-btn').onclick = async function() {
  const input = document.getElementById('conversation-input');
  const content = input.value.trim();
  if (!content || !currentDmId) return;
  const userId = window.auth.currentUser.uid;
  const messagesCol = collection(window.db, `artifacts/${window.appId}/users/${userId}/dms/${currentDmId}/messages`);
  await addDoc(messagesCol, {
    content,
    createdAt: serverTimestamp(),
    createdBy: userId,
    creatorDisplayName: window.currentUser?.displayName || 'You'
  });
  input.value = '';
};

// --- Start New DM ---
document.getElementById('start-conversation-btn').onclick = async function() {
  const userId = window.auth.currentUser.uid;
  const otherUid = prompt('Enter the UID of the user to DM:');
  if (!otherUid || otherUid === userId) return;
  const dmsCol = collection(window.db, `artifacts/${window.appId}/users/${userId}/dms`);
  const newDm = await addDoc(dmsCol, {
    participants: [userId, otherUid],
    createdAt: serverTimestamp(),
    lastActivity: serverTimestamp()
  });
  // Also create for the other user
  const otherDmsCol = collection(window.db, `artifacts/${window.appId}/users/${otherUid}/dms`);
  await addDoc(otherDmsCol, {
    participants: [userId, otherUid],
    createdAt: serverTimestamp(),
    lastActivity: serverTimestamp()
  });
