...
function testEmailPreview() {
  ...
  if (subjectInput && contentInput) {
    ...
    console.log('📝 Click the "Preview" button to test email preview functionality');
    return true;
  } else {
...
function testEmailTemplates() {
  ...
  const templates = Array.from(templateSelect.options).map(option => option.value);
...
function fillTestEmail() {
  ...
  const elements = {
    ...
    'email-content': 'Hello! This is a test email to verify the email system is working correctly.\n\nBest regards,\nThe Arcator Team'
  };
...
console.log('💡 Or use individual functions like emailSystemTests.fillTestEmail()'); 