    ...
    try {
      ...
      onAuthStateChanged(auth, async (user) => {
        ...
        if (user) {
          ...
          currentUser = null;
        }                                  a// firebase-init.js - Centralized Firebase Initialization and Exports
        import { initializeApp, getApps, getApp } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-app.js";
        import { getAuth, onAuthStateChanged, signInAnonymously, signInWithCustomToken } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-auth.js";
        import { getFirestore, doc, getDoc, setDoc, deleteDoc } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js";
...
// Promise to signal when Firebase has been initialized and is ready
        ...
        async function setupFirebaseAndUser() {
          ...
          if (getApps().length === 0) {
            ...
            // Attempt to get config from Canvas environment, strictly parsing if it's a string
            if (typeof __firebase_config !== 'undefined' && __firebase_config !== null) {
              try {
                // According to Canvas docs, __firebase_config should be a string to be parsed.
                // If it's an object, this parse will fail, indicating an environment inconsistency.
                finalFirebaseConfig = JSON.parse(__firebase_config);
                console.log("DEBUG: __firebase_config provided and parsed successfully.");
              } catch (e) {
                console.error("ERROR: Failed to parse __firebase_config as JSON. This might indicate that the Canvas environment provided it as an object instead of a string. Using hardcoded firebaseConfig. Error:", e);
                // Fallback to hardcoded if parsing fails or if it was already an object
